...
const Article = () => {
  ...
  const getRecentPost = async () => {
    try {
      const response = await axios.get(`http://localhost:1337/api/kahanis?populate=*`);
  ...
  const getCurrentPost = async () => {
    try {
      const response = await axios.get(`http://localhost:1337/api/kahanis/${id}?populate=*`);
      const data = response.data.data;
      setCurrentPost(data);
      console.log(data)
    } catch (error) {
  ...
  return (
    ...
    <div>
      ...
      <div className="bg-[#F3F3F3] px-20 italic leading-10 font-semibold text-[20px] py-5">
        {currentPost.storyline || "Loading story..."}
      </div>
